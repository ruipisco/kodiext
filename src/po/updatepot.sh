...
find $findoptions .. -name "*.py" -exec xgettext --no-wrap -L Python --from-code=UTF-8 -kpgettext:1c,2 --add-comments="TRANSLATORS:" -d kodiext -s -o kodiext-py.pot {} \+
$localgsed --in-place kodiext-py.pot --expression=s/CHARSET/UTF-8/
printf "Creating temporary file kodiext-xml.pot\n"
which python
if [ $? -eq 0 ]; then
	find $findoptions .. -name "setup.xml" -exec python xml2po.py {} \+ > kodiext-xml.pot
else
	find $findoptions .. -name "setup.xml" -exec python3 xml2po.py {} \+ > kodiext-xml.pot
fi
printf "Merging pot files to create: enigma2.pot\n"
cat kodiext-py.pot kodiext-xml.pot | msguniq -s --no-wrap --no-location -o kodiext.pot -
#printf "remove pot Creation date\n"
#$localgsed -i -e'/POT-Creation/d' kodiext.pot
printf "remove temp pot files\n"
rm kodiext-py.pot kodiext-xml.pot
